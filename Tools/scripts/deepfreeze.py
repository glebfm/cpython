...
class Printer:
    ...
    def generate_unicode(self, name: str, s: str) -> str:
        ...
        if kind == PyUnicode_1BYTE_KIND:
            ...
            datatype = "uint32_t"
        self.write(f"#ifndef {macro}")
        self.write(f"#define {macro} 1")
        self.write("static")
        with self.indent():
            with self.block("struct"):
                if ascii:
        ...
        with self.block(f"{name} =", ";"):
            if ascii:
                ...
                result = f"& {name}._compact._base.ob_base"

        self.write(f"#endif // {macro}")
        return result
    ...
    def generate_file(self, module: str, code: object)-> None:
        ...
        with self.block(f"static int\n{module}_init(void)"):
            for p in self.interns:
                with self.block(f"if ({p} < 0)"):
                    self.write("return -1;")
            self.write("return 0;")
        self.interns.clear()

        with self.block(f"static void\n{module}_fini(void)"):
            for p in self.deallocs:
                self.write(p)
        self.deallocs.clear()
...
PROLOGUE = """\
/* Auto-generated by Tools/scripts/deepfreeze.py */
#include "Python.h"
...
def generate_include(modname: str, infile: TextIO, output: TextIO) -> None:
    source = infile.read()
    printer = Printer(output)
    if is_frozen_header(source):
        code = decode_frozen_data(source)
    else:
       code = compile(source, f"<frozen {modname}>", "exec")
    printer.generate_file(modname, code)


def generate_deepfreeze(args: list[str], output: TextIO) -> None:
    printer = Printer(output)
    printer.write(PROLOGUE)
    modules = []
    for arg in args:
        incfile, modname = arg.rsplit(':', 1)
        modules.append(modname.replace(".", "_"))
        printer.write(f"#include \"{incfile}\"")
    with printer.block(f"void\n_Py_Deepfreeze_Fini(void)"):
        for module in modules:
            printer.write(f"{module}_fini();")
    with printer.block(f"int\n_Py_Deepfreeze_Init(void)"):
        for module in modules:
            with printer.block(f"if ({module}_init() < 0)"):
...
parser_include = subparsers.add_parser("include", help="Create deepfreeze include file from header file")
parser_include.add_argument("modname", type=str, help="module name")
parser_include.add_argument("infile", type=argparse.FileType("r", encoding="utf-8"), help="input header file")
parser_include.add_argument("outfile", type=argparse.FileType("w", encoding="utf-8"), help="output include file")

parser_deepfreeze = subparsers.add_parser("deepfreeze", help="Create main deepfreeze.c")
parser_deepfreeze.add_argument("-o", "--output", type=argparse.FileType("w", encoding="utf-8"), help="output deefreeze file")
parser_deepfreeze.add_argument('args', nargs="+", help="Input file and module name (required) in file:modname format")
...
def main() -> None:
    ...
    if args.command == "deepfreeze":
        filename = args.output.name
        try:
            with report_time("generate"):
                generate_deepfreeze(args.args, args.output)
        finally:
            args.output.close()
        if verbose:
            print(f"Wrote {os.path.getsize(filename)} bytes to {filename}")
    elif args.command == "include":
        try:
            generate_include(args.modname, args.infile, args.outfile)
        finally:
            args.infile.close()
            args.outfile.close()
