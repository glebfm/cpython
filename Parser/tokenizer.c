...
#if defined(MS_WINDOWS) || (defined(HAVE_FCNTL_H) && defined(F_DUPFD_CLOEXEC)) || defined(HAVE_DUP)
// working _Py_dup() function
static FILE *
...
// fall back to fdopencookie() custom I/O on wasm32-wasi
typedef union {
    void *cookie;
    int fd;
} borrowed;
...
#else
#error "need working _Py_dup() of fopencookie()
#endif
